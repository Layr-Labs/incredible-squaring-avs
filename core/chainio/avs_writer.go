...
type AvsWriterer interface {
	...
	SendNewTaskNumberToSquare(
		...
		quorumNumbers []byte,
	) (cstaskmanager.ICredibleSquaringTaskManagerTask, uint32, error)
	RaiseChallenge(
		ctx context.Context,
...
func NewAvsWriterFromConfig(c *config.Config) (*AvsWriter, error) {
	return NewAvsWriter(c.Signer, c.CredibleSquaringServiceManagerAddr, c.BlsOperatorStateRetrieverAddr, c.EthHttpClient, c.Logger)
}
...
func (w *AvsWriter) SendNewTaskNumberToSquare(ctx context.Context, numToSquare *big.Int, quorumThresholdPercentage uint32, quorumNumbers []byte) (cstaskmanager.ICredibleSquaringTaskManagerTask, uint32, error) {
	...
	if err != nil {
		...
		return cstaskmanager.ICredibleSquaringTaskManagerTask{}, 0, err
	}
	receipt := w.client.WaitForTransactionReceipt(ctx, tx.Hash())
	newTaskCreatedEvent, err := w.AvsContractBindings.TaskManager.ContractCredibleSquaringTaskManagerFilterer.ParseNewTaskCreated(*receipt.Logs[0])
	if err != nil {
...
func (w *AvsWriter) SendAggregatedResponse(ctx context.Context, task cstaskmanager.ICredibleSquaringTaskManagerTask, taskResponse cstaskmanager.ICredibleSquaringTaskManagerTaskResponse, nonSignerStakesAndSignature cstaskmanager.IBLSSignatureCheckerNonSignerStakesAndSignature) (*types.Receipt, error) {
	txOpts := w.Signer.GetTxOpts()
